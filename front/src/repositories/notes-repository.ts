...
import { NewNoteParams, NoteApiInterface } from '@/models/interfaces/api/notes';

export default class NotesRepository {
  static async create(params: NewNoteParams, sessionToken: string): Promise<Note> {
  ...
  static async fetch(sessionToken: string): Promise<Array<Note>> {
    return axios.get(this.pathTo(sessionToken)).then(response => response.data.map((body: NoteApiInterface) => new Note(body)));
  }
